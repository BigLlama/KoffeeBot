...
class Economy(commands.Cog):
    ...
    async def vote(self, ctx):
        ...
        embed.add_field(name='Enjoying KoffeeBot?', value="[Vote for me on top.gg!](https://top.gg/bot/901223515242508309/vote/)")
        await ctx.send(embed=embed)

    @commands.command(name="bal", aliases=['balance', 'wallet'])  # check balance command
    ...
    async def give(self, ctx, member: discord.Member, amount, item=None):
        db = sqlite3.connect('kof_db.sqlite')
        cursor = db.cursor()
        cursor.execute(f"SELECT * FROM ingredients INNER JOIN recipes USING (member_id) WHERE member_id = {ctx.author.id}")
        result = cursor.fetchone()
    ...
    async def steal(self, ctx, member: discord.Member):
        ...
        if result[1] < 500:
            self.client.get_command("steal").reset_cooldown(ctx)
            return await ctx.send("You need at least <:KoffeeKoin:939562780363726868> **500** in your wallet to rob someone")
        ...
        if result[1] < 500 or result[1] == 0:
            self.client.get_command("steal").reset_cooldown(ctx)
            return await ctx.send("Your target needs to have at least <:KoffeeKoin:939562780363726868> **500** in their wallet for you to rob "
                                  "them")
    ...
    async def dep(self, ctx, amount):
        ...
        if str(amount) == "all" or str(amount) == "max":
            ...
            await ctx.send(f"Successfully deposited <:KoffeeKoin:939562780363726868> **{'{:,}'.format(result[1])}** into your bank account")
    ...
    async def shop(self, ctx, category=1):
        ...
        if category == 2:
            ...
            embed.add_field(name="<:hat:984285090190360657> mafia hat <:KoffeeKoin:939562780363726868> **1,000,000**",
    ...
    async def job(self, ctx):
        ...
        embed.add_field(name='Babysitter 👶',
                        value='Requires: <:tea:959165836042588250>\nSalary: <:KoffeeKoin:939562780363726868> **5,000**', inline=False)
        ...
        embed.add_field(name='Programmer 💻',
                        value='Requires: <:rare_coffee:951551480215789658>\nSalary: <:KoffeeKoin:939562780363726868> **15,000**', inline=False)
    ...
    async def work(self, ctx, job=None):
        ...
        tired_chance = random.randint(1, 10)
        ...
        if job is None:
                ...
                if result[3] == joblist[i]:
                    ...
                    if tired_chance == 7:
                        if result[3] == "babysitter":
                            await ctx.send(f"You have used up all your <:tea:959165836042588250> `tea`. Without it you cannot work\n"
                            ...
                            await ctx.send(f"You have used up all your <:rare_coffee:951551480215789658> `rcoffee`. Without it you cannot work\n"
                                        "You will need to craft a new form of energy and start looking for a job again")
                            update_job(ctx.author, 'unemployed')
            ...
            if result[6] < 1:
                self.client.get_command("work").reset_cooldown(ctx)
                return await ctx.send("You need to craft at least 1 <:tea:959165836042588250> `tea` to qualify for this job")
            else:
    ...
    async def daily(self, ctx):
        open_wallet(user=ctx.author)
        daily_amount = 25000
        await ctx.send(f"You have recieved your daily amount of <:KoffeeKoin:939562780363726868> **{'{:,}'.format(daily_amount)}**")
    ...
    async def slots(self, ctx, amount):
        ...
        em1 = discord.Embed(title="=-=-=-=-=-=\n"
                             ...
                             "=-=-=-=-=-=", color=discord.Color.orange())
        em1.set_author(name="Slot machine", icon_url=ctx.author.avatar_url)
        em2 = discord.Embed(title=f"=-=-=-=-=-=\n"
                             f"{one} :red_square: :red_square: :round_pushpin:\n"
                             "=-=-=-=-=-=", color=discord.Color.orange())
        em2.set_author(name="Slot machine", icon_url=ctx.author.avatar_url)
        em3 = discord.Embed(title=f"=-=-=-=-=-=\n"
                             f"{one} {two} :red_square: :round_pushpin:\n"
                             "=-=-=-=-=-=", color=discord.Color.orange())
        em3.set_author(name="Slot machine", icon_url=ctx.author.avatar_url)
        em4 = discord.Embed(title=f"=-=-=-=-=-=\n"
                             f"{one} {two} {three} :round_pushpin:\n"
                             "=-=-=-=-=-=", color=discord.Color.orange())
        em4.set_author(name="Slot machine", icon_url=ctx.author.avatar_url)
        ...
        if one == two == three:
            ...
            embed = discord.Embed(description=f"Congratulations!!! {ctx.author.mention}\nYou won <:KoffeeKoin:939562780363726868> **{'{:,}'.format(win)}**!", color=discord.Color.orange())
            embed.set_author(name="Slot machine", icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
        else:
            remove_bal(ctx.author, amount)
    ...
    async def rich(self, ctx):
        ...
        if len(valid) < 10:
            return await ctx.send(
                "Atleast 10 people in this server need a KoffeeBot account/balance\nYou can open an account by typing `kof bal`")
    ...
    async def mafia(self, ctx, action=None, target=None):
        ...
        if action == 'create': # creating a mafia
            ...
            cursor.execute(f"SELECT * FROM wallets WHERE member_id = {user.id}")
            result = cursor.fetchone()
            if result[1] < 500000: # insufficient cash
                return await ctx.send("You need <:KoffeeKoin:939562780363726868>**500,000** to create a mafia")
            cursor.execute(f"SELECT * FROM mafia WHERE member_id = {user.id}")
            result = cursor.fetchone()
            ...
            if result[1] == 'none':
                ...
                return await ctx.send("You cannot leave your own mafia. Either promote someone else to `Boss` or disband the entire mafia")
            ...
            try:
                ...
                if msg.content.startswith("decline"):
                    return await ctx.send(f"{user.mention}, {target.name} has declined your invitation to join {result[1]}")
            ...
            mafname = result[1]
            if result[1] == 'none':
                return await ctx.send("You aren't currently in any mafia. Join or create a mafia to view your mafia stats")
            if result[2] != "Boss":
            ...
            embed = discord.Embed(title=f"<:hat:984285090190360657> {result[1]} Mafia <:hat:984285090190360657>", color=discord.Color.orange())
            embed.set_thumbnail(
                url='https://cdn.discordapp.com/attachments/922598156842172508/923140639556775946/koffee4.png')
            embed.add_field(name='Profile', value=f"**`Title:`** {result[2]}\n**`Heists completed:`** {result[3]}", inline=True)

            cursor.execute(f"SELECT * FROM mafias WHERE mafia_name = '{maf_name}'")
            result = cursor.fetchone()
            embed.add_field(name='Vault', value=f"<:KoffeeKoin:939562780363726868>{'{:,}'.format(result[1])}", inline=False)
    ...
    async def vault(self, ctx, action, amount):
        ...
        cursor.execute(f"SELECT * FROM wallets LEFT JOIN mafias WHERE member_id = {ctx.author.id} AND mafia_name = '{mafname}'")
        result = cursor.fetchone()
    ...
    async def on_message(self, message):
        ...
        if message.channel.id == 979427519742759002:
            ...
            embed.set_thumbnail(
                url='https://cdn.discordapp.com/attachments/922598156842172508/923140639556775946/koffee4.png')
            embed.add_field(name="Thank you for voting for me :D", value=f"You have received <:KoffeeKoin:939562780363726868>**25,000**")
